            ...
            <div className=" mb-4 w-full">
                {label && (
                    <label htmlFor={id} className={labelBaseStyles}>
                        {label}{" "}
                        {required && (
                            <span className={requiredIndicatorStyles}>*</span>
                        )}
                    </label>
                )}
                ...
                <div className="relative">
                    <div
                        className={cn(
                            ...
                            "h-auto min-h-[44px] flex items-center flex-wrap gap-1.5 p-2 cursor-text"
                        )}
                    ...
                    >
                        {selectedValues.map((value) => (
                            <span
                                key={value}
                                className="flex items-center bg-white/50 text-slate-700 text-xs px-2 py-1 rounded-md border border-white/70"
                            >
                                {selectedOptionsMap[value]}
                                <button
                                    type="button"
                                    className="ml-1.5 text-slate-600 hover:text-slate-900 focus:outline-none"
                        ...
                        <input
                            ...
                            className="flex-grow bg-transparent outline-none text-slate-800 text-sm h-full"
                            {...props}
                        />
                    </div>
                    {isOpen && (
                        <div className="light-glass-pane absolute z-10 w-full mt-2 rounded-lg animate-in fade-in-0 zoom-in-95 p-1">
                            <ul className="max-h-60 overflow-y-auto">
                                {options.map((option) => (
                                    <li
                                        key={option.value}
                                        className="flex items-center p-2 rounded-md hover:bg-white/30 cursor-pointer"
                                        onClick={() =>
                                            handleSelectOption(option.value)
                                        }
                                    >
                                        <div
                                            className={cn(
                                                "h-4 w-4 shrink-0 rounded border border-slate-500 flex items-center justify-center",
                                                {
        ...
        return (
            <div className="mb-4 w-full">
                ...
                <label
                    htmlFor={id}
                    className={cn(
                        "light-glass-pane flex flex-col items-center justify-center w-full min-h-[120px] p-4 text-center rounded-lg border-2 border-dashed border-white/50 cursor-pointer hover:border-orange-500/50 hover:bg-white/60 transition-all duration-300",
...
const RadioGroup = React.forwardRef(
    (
        ...
        <fieldset className="mb-4 w-full">
            {label && (
                <legend className={labelBaseStyles}>
                    {label}{" "}
                    {required && (
                        <span className={requiredIndicatorStyles}>*</span>
                    )}
            ...
            <div className="flex flex-col space-y-2 mt-2">
                {options.map((option, index) => (
                    <label
                        key={option.value}
                        className="flex items-center cursor-pointer p-2 rounded-md hover:bg-white/20"
                    >
                        <input
                            type="radio"
                            ...
                            onChange={onChange}
                            className="h-5 w-5 shrink-0 cursor-pointer appearance-none rounded-full border-2 border-slate-900/30 bg-white/20 checked:border-transparent checked:bg-gradient-to-br from-orange-500 to-amber-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white/50 focus:ring-orange-500"
                            ref={index === 0 ? ref : null}
                            {...props}
                        />
                        <span className="ml-3 text-slate-800 text-sm font-medium">
                            {option.label}
                        </span>
                    </label>
                ))}
...
const Checkbox = React.forwardRef(
    (
        { label, id, name, checked, onChange, error, required, ...props },
        ref
    ) => (
            ...
            >
                <div className="relative flex items-center h-5 w-5">
                    <input
                        ...
                        checked={checked}
                        onChange={onChange}
                        className="h-5 w-5 shrink-0 cursor-pointer appearance-none rounded-md border-2 border-slate-900/30 bg-white/20 checked:border-transparent checked:bg-gradient-to-br from-orange-500 to-amber-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white/50 focus:ring-orange-500"
                        ref={ref}
                        {...props}
                    />
                    {checked && (
                        <Check className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-4 w-4 text-white pointer-events-none" />
                    )}
                </div>
                <span className="ml-3 text-slate-800 text-sm font-medium">
                    {label}
                ...
                {required && (
                    <span className={cn(requiredIndicatorStyles, "ml-1")}>
                        *
                    </span>
                )}
            </label>
